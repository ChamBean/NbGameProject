...
class SceneManager {
	...
	public myRoleData:PlayerInfoData = null;
	...
	public createMyRole(roleData:PlayerInfoData):void
	{
		this._myRole = this.mapViewMediator.createMyRole(roleData);
		this.myRoleData = this._myRole.roleData as PlayerInfoData;
	}
	...
	private onAutoMapSearch(e:egret.Event):void{
		if(e.data != null)
	...
	private static roleIndex:number = 0;
	private randomRole():void{
		for(var i:number = 0;i < 50;i++)
		{
			var player:Player = this.mapViewMediator.createOtherPlayer();
	...
	private onChangeFeature(e:egret.Event):void{
		var key:string = e.data.key;
		var roleData:PlayerInfoData = this.myRoleData;
		if(key == 'sex'){
