class MapVo {
	...
	public tileArray:				Array<Array<string>>;//切片名称数组(真实http的图片地址),请按图片位置组织为二维数据
	...
	public nodeArray:				Array<Array<MapNode>> ;	//可行走数据
	...
	public startNode:MapNode;
	public endNode:MapNode;

	public dispose():void
	{
		if(this.nodeArray != null) 
		{
			for(var i:number = 0;i < this.nodeArray.length;i++){
				for(var j:number = 0;j < this.nodeArray[i].length;j++){
	 ...
	 * 
	 */
	public getNodeByXY(px:number, py:number):MapNode
	{
	...
	public getNode(px:number,py:number):MapNode
	{
		px = Math.floor(px);
		py = Math.floor(py);
	 ...
	 * @return 
	 */		
	public static transNodePoint(node:MapNode,p:egret.Point = null):egret.Point
	{
		if(p == null)
		{
	 ...
	 * @return 
	 */		
	public static getPoint(node:MapNode,px:number = 0,py:number = 0):egret.Point
	{
